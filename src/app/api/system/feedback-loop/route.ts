...
export async function GET(request: NextRequest) {
  try {
    ...
    const metrics: Record<string, { total: number; success: number; failed: number; avgDurationMs: number }> = {};
    for (const run of agentStats || []) {
      const name = run.agent_name;
      if (!metrics[name]) metrics[name] = { total: 0, success: 0, failed: 0, avgDurationMs: 0 };
      metrics[name].total++;
      if (run.status === 'completed') metrics[name].success++;
      if (run.status === 'failed') metrics[name].failed++;
      if (run.duration_ms) {
        metrics[name].avgDurationMs = 
          (metrics[name].avgDurationMs * (metrics[name].total - 1) + run.duration_ms) / metrics[name].total;
      }
    }
    ...
    return NextResponse.json({
      ...
      agentHealth,
      systemUptime: agentHealth.every(a => a.successRate > 80) ? 'healthy' : 'degraded',
    });
  } catch (error) {
